...
class Import {
 public:
  ...
  void add_import_star(std::string filename) {
    namespaces_imports_.emplace(std::move(filename));
  }

  std::map<std::string, std::set<ImportName>> get_regular_imports() const {
    return regular_imports_;
  }

  std::set<std::string> get_namespaces_imports() const {
